...
function Header() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const shouldShow = currentScrollPos < scrollThreshold;
      setShowBanner(shouldShow);
      // Dispatch event with visibility state
      window.dispatchEvent(new CustomEvent('referralBannerVisibility', { detail: shouldShow }));
  ...
  useEffect(() => {
    ...
    window.addEventListener('referralBannerVisibility', handleBannerVisibility);
    return () => {
      window.removeEventListener('referralBannerVisibility', handleBannerVisibility);
    };
  }, []);
  ...
  return (
    <>
      <ReferralBanner />
      <header
        className={`w-full fixed left-0 right-0 z-40 transition-all duration-300 ${
          scrolled
